...
if [[ "$1" == "--status" || "$1" == "-s" ]]; then
  ...
  if [ -f "lib/server/auth/db.ts" ]; then
    ...
    if [ -f "app/interactive-hub/page.tsx" ]; then
      echo "✅ 互动展厅: 已启用「互动实验与游戏工坊」展厅 (/interactive-hub)"
    fi
  else
...
if [[ "$1" == "--revert" || "$1" == "--uninstall" ]]; then
  ...
  if git apply -R --check --whitespace=nowarn "$PATCH_FILE" 2>/dev/null; then
    ...
    rm -rf app/api/auth app/api/admin components/auth components/admin lib/server/auth lib/store/auth-store.ts tests/auth scripts/reset-admin-password.mjs README-MULTIUSER.md app/my-courses app/interactive-hub app/api/interactive-library lib/server/interactive-library
