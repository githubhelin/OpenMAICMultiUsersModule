...
跳过繁琐的交互式人机对话，纯粹专注于课件资料批量全自动制作的生产级扩展模块：

- **详细指南文档**：👉 [batch-course-studio/README.md](./batch-course-studio/README.md)
- **核心特性**：
  - **多格式文件提取**：支持 PPTX（Worker 工作线程沙箱隔离解析）、PDF / DOCX（原生调用已配置的 MinerU 高精解析）、TXT、Markdown 等。
  - **继承系统全局配置**：自动继承管理员配置的默认大模型（DeepSeek、Claude、OpenAI、Qwen、Gemini 等）、语音合成（TTS）、视觉生图。
  - **双重制课模式**：
    - **多课件融合为一门课**：提炼多文件核心要点，融合生成一门长篇连贯交互大课。
    - **独立课件批量生成**：多文件独立并发批量制作，一键处理整学期课程。
  - **真·深度交互模式 (Interactive Mode)**：
    - 深度打通官方 `PROMPT_IDS.INTERACTIVE_OUTLINES` 模版，硬性规约**全课 70% 以上必须为互动场景**（物理/科学仿真模拟器 `simulation`、趣味闯关小游戏 `game`、动态结构图解 `diagram` 等），仅保留 30% 用于导读与总结；
    - 内置防降级保护机制（Fallback Protection），严格校验并补全交互组件参数，杜绝大模型输出缺失字段导致误降级为幻灯片。
  - **任务即时中断与 Token 资源止损**：
    - 运行中随时一键中断任务，毫秒级终止网络与大模型请求，未生成课件全部取消，杜绝 Token 浪费。
  - **队列文件精准移除与历史记录管理**：
    - 排队中的课件可单个移出队列跳过生成，历史批次支持一键安全删除并自动清理临时缓存文件。
  - **全后台异步流水线 + 实时多任务大屏 (`/batch-studio`)**。
  - **多用户云端漫游无缝集成**：课程自动落库至所属用户的多用户存储中，多端自动同步。
  - **标准 REST API 接口**：支持提交任务、查询进度、全局中断、队列移除与记录删除，方便脚本批处理与系统集成。
