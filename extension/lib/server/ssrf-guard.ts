 ...
 */
function isFakeIp(canonical: string): boolean {
  try {
...
function isNeverAllowedRange(value: string): boolean {
  const canonical = canonicalizeIp(value);
  if (canonical === null) return false;
  if (isFakeIp(canonical)) return false;
 ...
 * link-local targets, plus carrier-grade NAT (100.64.0.0/10) and transparent
 * proxy Fake-IP pools (198.18.0.0/15).
 */
function isOptInGovernedRange(value: string): boolean {
  const canonical = canonicalizeIp(value);
  if (canonical === null) return false;
  if (isFakeIp(canonical)) return true;
