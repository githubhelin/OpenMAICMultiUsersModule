...
export async function resolveModel(params: {
  ...
  // For server-managed providers, ensure the model is allowed by server config (e.g. OPENAI_MODELS).
  // If the client sent an obsolete or unconfigured model, gracefully fall back to server's current model.
  let effectiveModelId = modelId;
  if (managed) {
    const validatedModelId = resolveLLMModel(providerId, modelId);
    if (validatedModelId && validatedModelId !== modelId) {
      effectiveModelId = validatedModelId;
      modelString = `${providerId}:${effectiveModelId}`;
    }
  }
