...
async function fetchRemoteStageDocument(stageId: string): Promise<AppDocument | null> {
  try {
    ...
    if (res.ok) {
      ...
      if (data && data.stage) {
        return {
          ...
          outline: data.outline as AppDocumentOutline | undefined,
        };
      }
    }
    const classRes = await fetch(`/api/classroom?id=${encodeURIComponent(stageId)}`, {
      credentials: 'include',
    });
    if (classRes.ok) {
      const classData = await classRes.json();
      if (classData && classData.classroom && classData.classroom.stage) {
        return {
          stage: classData.classroom.stage as Stage,
          scenes: (classData.classroom.scenes || []) as Scene[],
        };
      }
    }
  } catch {
  ...
  try {
    await Promise.all(
      stageIds.map(async (stageId) => {
        let document = (await accessDocument(stageId)).document;
        if (!document) {
          const remote = await fetchRemoteStageDocument(stageId);
          if (remote) {
            document = remote;
...
export async function renameStage(stageId: string, newName: string): Promise<void> {
  try {
    try {
      ...
      // Local Dexie record not found (e.g. cloud stage)
    }
    fetch(`/api/stages/${encodeURIComponent(stageId)}`, {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
...
export async function stageExists(stageId: string): Promise<boolean> {
  try {
    ...
    if ((await getLegacyDocumentStore().read(stageId)) !== null) return true;
    try {
      const res = await fetch(`/api/stages/${encodeURIComponent(stageId)}`, { credentials: 'include' });
