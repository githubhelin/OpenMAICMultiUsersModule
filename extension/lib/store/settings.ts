...
/**
 * Prune stage routes that are operator-only or point to models no longer available
 * on the configured provider. If an operator updates environment models (e.g. from
 * 3.7 to 3.8, or in the future to 4.0), stale stage routes are automatically purged
 * so the stages seamlessly fall back to following the current mainline model.
 */
export function pruneInvalidStageRoutes(
  routes: SettingsState['llmStageRoutes'] | undefined,
  providersConfig: ProvidersConfig | undefined,
): SettingsState['llmStageRoutes'] | null {
  if (!routes) return null;
  let changed = false;
  const filtered: SettingsState['llmStageRoutes'] = {};
  ...
  for (const [stage, selection] of Object.entries(routes)) {
    ...
    // Operator-only stage: cannot be user-routed
    if (stage === 'maic-agent-driver') {
      changed = true;
      continue;
    }
    const provider = providersConfig?.[selection.providerId];
    // Provider disabled or removed
    if (!provider || provider.enabled === false) {
      changed = true;
      continue;
    }

    // Check if the selected model is still in the provider's allowed models list
    ...
    filtered[stage] = selection;
  }

  return changed ? filtered : null;
}

export const pruneOperatorOnlyStageRoutes = pruneInvalidStageRoutes;
...
/**
 * Automatically migrate legacy model references (e.g. gemini-3.7-flash-high -> gemini-3.8-flash-high).
 * Ensures old client-side persisted state doesn't keep calling deprecated models.
 */
export function migrateLegacyModelIds(state: Partial<SettingsState>): void {
  if (state.modelId === 'gemini-3.7-flash-high') {
    state.modelId = 'gemini-3.8-flash-high';
  }

  if (state.llmStageRoutes) {
    const updatedRoutes: typeof state.llmStageRoutes = { ...state.llmStageRoutes };
    let changed = false;
    for (const [stage, route] of Object.entries(updatedRoutes)) {
      if (route && route.modelId === 'gemini-3.7-flash-high') {
        updatedRoutes[stage] = { ...route, modelId: 'gemini-3.8-flash-high' };
        changed = true;
      }
    }
    if (changed) {
      state.llmStageRoutes = updatedRoutes;
    }
  }

  if (state.providersConfig) {
    for (const providerCfg of Object.values(state.providersConfig)) {
      if (providerCfg && Array.isArray(providerCfg.models)) {
        let changed = false;
        const newModels = providerCfg.models.map((m) => {
          if (m.id === 'gemini-3.7-flash-high') {
            changed = true;
            return {
              ...m,
              id: 'gemini-3.8-flash-high',
              name:
                m.name === 'gemini-3.7-flash-high'
                  ? 'gemini-3.8-flash-high'
                  : m.name.replace(/3\.7/g, '3.8'),
            };
          }
          return m;
        });
        if (changed) {
          ...
          providerCfg.models = newModels.filter((m) => {
  ...
  if (state.thinkingConfigs) {
    const updatedThinking = { ...state.thinkingConfigs };
    let changed = false;
    for (const [key, val] of Object.entries(updatedThinking)) {
      if (key.includes('gemini-3.7-flash-high')) {
        const newKey = key.replace(/gemini-3\.7-flash-high/g, 'gemini-3.8-flash-high');
        delete updatedThinking[key];
        updatedThinking[newKey] = val;
        changed = true;
      }
    }
    if (changed) {
      state.thinkingConfigs = updatedThinking;
    }
  }
}
  ...
  persist(
        ...
        fetchServerProviders: async () => {
          try {
            ...
            set((state) => {
              ...
              const prunedStageRoutes = pruneInvalidStageRoutes(
                state.llmStageRoutes,
                newProvidersConfig,
              );

              return {
                providersConfig: newProvidersConfig,
                ...(prunedStageRoutes ? { llmStageRoutes: prunedStageRoutes } : {}),
    ...
    },
    {
      ...
      migrate: (persistedState: unknown, version: number) => {
        ...
        const prunedInvalid = pruneInvalidStageRoutes(state.llmStageRoutes, state.providersConfig);
        if (prunedInvalid) state.llmStageRoutes = prunedInvalid;
      ...
      merge: (persistedState, currentState) => {
        ...
        migrateLegacyModelIds(merged as Partial<SettingsState>);
        ...
        const prunedInvalid = pruneInvalidStageRoutes(
          typedMerged.llmStageRoutes,
          typedMerged.providersConfig,
        );
        if (prunedInvalid) typedMerged.llmStageRoutes = prunedInvalid;
